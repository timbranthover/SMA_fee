...
- Proposal impact recalculates from the actual selected investments and dollar allocations, including direct source-stock repurchases and unallocated cash. Estimates use declared asset classes; unknown exposures are withheld, underlying holdings and tax liability are not modeled, and legacy drafts require reopening the household decision to refresh their estimates. Proposal controls use the canonical scenario response.
- Illustrative SMA records have unique manager/strategy identities with aligned benchmarks, objectives, fees, minimums and strategy flags. These records are not actual manager product terms.
...
- Result columns adapt to the selected vehicle and enrich the visible Equity/ETF rows from Yahoo Finance in one bounded post-render batch. Equities use live/current price, daily change, intraday series, market cap, 1Y return, 3Y annualized return, forward P/E and dividend yield; ETFs use live/current price, daily change, intraday series, net assets/AUM, 1Y return, 3Y annualized return and net expense ratio. Existing illustrative values remain fail-soft fallbacks when a provider field is unavailable. Users can choose, order and persist up to five valid fields or apply a vehicle-specific Research, Performance, Income, Risk, or Cost preset; incompatible fields are excluded by configuration.
