...
function policyComparisonMarkup(review) {
  const maximum = Math.max(30, Math.ceil(review.holding.weight / 5) * 5);
  const excess = Math.max(0, review.holding.weight - review.targetWeight);
  return `<div class="policy-comparison" role="group" aria-label="Position exposure compared with household target"><div class="policy-comparison-row"><div><span>Current position</span><strong>${review.holding.weight.toFixed(1)}%</strong></div><progress class="policy-current-progress" max="${maximum}" value="${review.holding.weight}" aria-label="Current position ${review.holding.weight.toFixed(1)} percent"></progress></div><div class="policy-comparison-row"><div><span>Household target</span><strong>${review.targetWeight.toFixed(1)}%</strong></div><progress class="policy-target-progress" max="${maximum}" value="${review.targetWeight}" aria-label="Household target ${review.targetWeight.toFixed(1)} percent"></progress></div><p><strong>+${excess.toFixed(1)} percentage points</strong> above the household target · ${formatWealthCurrency(review.targetRelease)} modeled release to reach it</p></div>`;
}

function concentrationDrawer(review, decisionSummary) {
  const decision = decisionSummary?.decisions?.find((candidate) => candidate.kind === "concentration");
  ...
  return `<header class="wealth-drawer-header"><div><span class="eyebrow">PORTFOLIO RISK · ${escapeHtml(HOUSEHOLD.name.toUpperCase())}</span><button type="button" class="wealth-drawer-back" data-close-wealth-drawer>${backLabel("Back to Total Wealth")}</button></div><button type="button" class="wealth-drawer-close" data-close-wealth-drawer aria-label="Close concentration review">×</button></header>
    <div class="wealth-drawer-body">
      ...
      <section class="concentration-section"><div class="section-heading"><span>Exposure</span><h3>Position relative to policy</h3></div>${policyComparisonMarkup(review)}</section>
      ...
      ${decision ? `<section class="concentration-next"><div><span class="panel-kicker">NEXT STEP</span><h3>Model a reduction in ${escapeHtml(review.holding.symbol)}</h3><p>Review the sale and estimated gain, then select investments and build a client proposal.</p></div><button type="button" class="primary-button" data-decision-open="${escapeHtml(decision.id)}">${getProposal(decision.id)?.status === "Ready for client" ? "View proposal" : "Model this change"} <span aria-hidden="true">→</span></button></section>` : ""}
    </div>`;
}
...
function formatMeetingActivity(item) {
  const date = formatDecisionDate(item.occurredAt);
  const detail = String(item.detail || "").startsWith(date)
    ? String(item.detail).slice(date.length).replace(/^\s*·\s*/, "")
    : String(item.detail || "");
  return `${escapeHtml(date)}${detail ? ` · ${escapeHtml(detail)}` : ""}`;
}
...
function meetingBriefDrawer(data) {
  ...
  return `<header class="wealth-drawer-header"><div><span class="eyebrow">MEETING PREP · ${escapeHtml(HOUSEHOLD.name.toUpperCase())}</span><button type="button" class="wealth-drawer-back" data-close-wealth-drawer>← Back to Total Wealth</button></div><button type="button" class="wealth-drawer-close" data-close-wealth-drawer aria-label="Close meeting brief">×</button></header><div class="wealth-drawer-body meeting-brief"><div class="drawer-section-heading"><span class="panel-kicker">RELATIONSHIP BRIEF</span><h2 id="wealthDrawerTitle">Prepare the conversation</h2><p>${escapeHtml(data.household.members.join(" · "))} · Last planning review ${escapeHtml(data.household.lastPlanningReview)}</p></div><section><h3>Current household</h3><div class="meeting-metric-grid">${data.changes.map((item) => `<div><span>${escapeHtml(item.label)}</span><strong>${typeof item.value === "number" ? (item.label === "Investable cash" ? formatWealthCurrency(item.value) : formatSignedWealthCurrency(item.value)) : escapeHtml(item.value)}</strong></div>`).join("")}</div></section><section><h3>Open decisions</h3><div class="meeting-decision-list">${decisionRows || `<p class="drawer-empty">No active decisions.</p>`}</div></section><section><h3>Upcoming</h3><div class="meeting-copy-list">${(data.upcoming || []).map((item) => `<div><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.detail)}</span></div>`).join("") || `<p class="drawer-empty">No material upcoming obligations.</p>`}</div></section><section><h3>Recent relationship activity</h3><div class="meeting-copy-list">${(data.recentActivity || []).slice(0, 5).map((item) => `<div><strong>${escapeHtml(item.title)}</strong><span>${formatMeetingActivity(item)}</span></div>`).join("")}</div></section></div>`;
}
...
async function openWealthDrawer(id) {
  ...
  if (id === "concentration") detailRequest = Promise.all([loadConcentrationReview(state.currentHouseholdId), loadDecisionSummary(state.currentHouseholdId)]).then(([review, summary]) => review ? concentrationDrawer(review, summary) : operationalDrawer("relationship"));
...
function decisionPlanMarkup(detail, plan) {
  if (!plan) return `<div class="decision-plan-empty"><span class="panel-kicker">ACTION PLAN</span><h3>Next steps</h3><p>${detail.decision.implementationType === "none" ? "Schedule the funding source and timing to save a plan. Mark funded only when the obligation has been met." : "Select investments to save an implementation basket. The decision stays in Reviewing until there is a real plan."}</p></div>`;
...
function renderDecisionStudio() {
  ...
  updateHtml(el("decisionStudioContent"), `<header class="decision-studio-header"><button type="button" class="decision-back" data-close-decision-studio>← ${escapeHtml(HOUSEHOLD.name)}</button><div><span class="eyebrow">DECISION STUDIO · ${escapeHtml(detail.decision.priority.toUpperCase())}</span><h2 id="decisionStudioTitle">${escapeHtml(detail.decision.title)}</h2><p>${escapeHtml(detail.decision.objective)}</p></div><span class="decision-status">${escapeHtml(status)}</span><button type="button" class="decision-close" data-close-decision-studio aria-label="Close decision studio">×</button></header><div class="decision-studio-body"><aside class="decision-facts"><div class="decision-signal tone-${status === "Complete" ? "neutral" : escapeHtml(detail.decision.tone)}"><span>${escapeHtml(detail.evidence.severity)}</span><strong>${escapeHtml(detail.evidence.title)}</strong><p>${escapeHtml(detail.evidence.detail)}</p><small>${escapeHtml(detail.evidence.source)}</small></div><section><span class="panel-kicker">WHAT WE KNOW</span><div class="decision-fact-list">${detail.facts.map((fact) => `<div><span>${escapeHtml(fact.label)}</span><strong>${decisionValue(fact.value, fact.label)}</strong></div>`).join("")}</div></section><section class="decision-assumptions"><span class="panel-kicker">MODEL ASSUMPTIONS</span>${scenario.assumptions.map((assumption) => `<p>${escapeHtml(assumption)}</p>`).join("")}</section></aside><main class="decision-model"><div class="decision-model-inner"><div class="decision-model-heading"><span class="panel-kicker">WHAT COULD CHANGE</span><h3>Model the household consequence</h3><p>Adjust only explicit assumptions. The resulting changes are calculated from this household's current data.</p></div>${decisionScenarioControls(detail, scenario)}${affectedAccounts}<div class="decision-consequence-heading"><span class="panel-kicker">HOUSEHOLD CONSEQUENCE</span><h3>Before and after</h3></div>${decisionScenarioOutcomes(detail, scenario)}</div></main><aside class="decision-plan-column">${implementationCard}${decisionPlanMarkup(detail, plan)}</aside></div>`);
}
...
async function openDecisionStudio(decisionId, { preserve = false, inputs = null } = {}) {
  ...
  if (!el("wealthDrawer").classList.contains("open")) state.lastFocus = document.activeElement;
  closeWealthDrawer({ restoreFocus: false });
  ...
  try {
    ...
    if (getDecisionWorkflowStatus(decisionId, detail.decision.status) === "New") {
      ...
      renderWealthWorkspace();
      renderBookRows();
      state.lastFocus = document.querySelector(`[data-wealth-insight="${CSS.escape(detail.decision.sourceInsightId)}"]`) || state.lastFocus;
    }
...
function proposalFundingSource(detail) {
  const evidence = detail?.evidence?.title || "Confirm the funding account before implementation";
  if (detail?.decision?.kind === "concentration") {
    const position = String(detail.facts.find((fact) => fact.label === "Position")?.value || "Concentrated position").split(" · ")[0];
    return { label: `${position} sale proceeds`, value: evidence };
  }
  if (detail?.decision?.kind === "liquidity" || detail?.decision?.kind === "allocation") return { label: "Household cash", value: evidence };
  return { label: "Household portfolio assets", value: evidence };
}
...
function launchDecisionImplementation() {
  ...
  launchInvestmentContext({
    ...
    title: detail.decision.kind === "concentration" ? `Reinvest ${detail.model.sourceSymbol} sale proceeds` : `Select investments for ${HOUSEHOLD.name}`,
    ...
    sourceLabel: fundingSource.label,
    sourceValue: fundingSource.value,
    impact: proposalImpactFromScenario(detail, scenario),
    impactModel: scenario.proposalModel,
  });
}
...
function openPrimaryConcentrationDecision() {
  openWealthDrawer("concentration");
}
...
async function refreshEmbeddedMandate(updates) {
  ...
  try {
    ...
    state.householdScenario.impactModel = scenario.proposalModel;
    ...
    if (controller === state.decisionScenarioController) { state.mandatePending = false; renderProposalTray(); }
  }
}

function showScenarioRibbon({ source, title, tags, decisionId = null, implementationAmount = 0, proposalMode = false, decisionTitle = "", objective = "", sourceLabel = "", sourceValue = "", impact = {}, impactModel = null }) {
  state.householdScenario = { source, title, tags, householdId: state.currentHouseholdId, householdName: HOUSEHOLD.name, decisionId, implementationAmount, proposalMode, decisionTitle, objective, sourceLabel, sourceValue, impact, impactModel };
  ...
  el("scenarioRibbon").classList.toggle("proposal-command-mode", proposalMode);
...
function handleWealthInsight(id) {
  ...
  if (decision) { if (getProposal(decision.id)?.status === "Ready for client") openProposalBuilder(decision.id); else if (decision.kind === "concentration") openWealthDrawer("concentration"); else openDecisionInScreener(decision.id); return; }
  const insight = HOUSEHOLD_INSIGHTS.find((candidate) => candidate.id === id);
  if (!insight) return;
  if (insight.action.type === "concentration") { openWealthDrawer("concentration"); return; }
  if (insight.action.type === "investment-search") {
    const householdId = state.currentHouseholdId;
    loadDecisionSummary(householdId).then((summary) => {
      if (state.currentHouseholdId !== householdId) return;
      const linked = summary.decisions.find((item) => item.sourceInsightId === id);
      if (linked) openDecisionInScreener(linked.id);
      else showToast("This investment decision is unavailable");
    }).catch(() => showToast("Unable to open the investment decision"));
    return;
  }
...
function renderResults() {
  ...
  body.innerHTML = state.items.map((item) => {
    ...
    return `<tr data-row-id="${escapeHtml(item.id)}" class="${proposed ? "proposal-selected-row" : ""}">
      ...
      <td><div class="investment-cell">${productMark(item)}<div class="investment-meta"><a href="${escapeHtml(profileHref(item))}" data-detail-id="${escapeHtml(item.id)}">${escapeHtml(item.name)}</a><div class="investment-sub">${escapeHtml(item.type)} · ${escapeHtml(item.manager)}${item.category === "SMAs" ? `<span class="sma-role">${escapeHtml(item.objective)} · ${escapeHtml(item.benchmark || "Benchmark pending")}</span>` : ""}${item.matchReason ? `<span class="match-reason">${escapeHtml(item.matchReason)}</span>` : ""}<span class="badges">${researchStatus(item.researchStatus)}${visibleFlags(item.flags).map(badge).join("")}</span></div></div></div></td>
...
function proposalTargetAmount() {
  return Math.max(0, Math.round(Number(state.householdScenario?.implementationAmount ?? state.proposal?.totalAmount ?? 0)));
}
...
function renderProposalTray() {
  ...
  const preview = el("proposalImpactPreview");
...
function proposalImpactValue(value, format) {
  const numeric = Number(value);
  if (value === null || value === undefined || !Number.isFinite(numeric)) return "Needs exposure data";
...
function openProposalBuilder(decisionId = state.householdScenario?.decisionId) {
  ...
  if (!state.householdScenario?.proposalMode) {
    state.householdScenario = {
      ...
      impactModel: proposal.impactModel,
    };
  }
...
async function returnToProposalSelection() {
  const context = state.householdScenario;
  if (!context?.proposalMode) return;
  setWorkspaceView("investments");
  showScenarioRibbon(context);
  if (state.activeDecisionScenario?.decisionId === context.decisionId || !context.impactModel?.scenarioInputs) return;
  const controller = new AbortController();
  state.decisionScenarioController?.abort();
  state.decisionScenarioController = controller;
  state.mandatePending = true;
  ...
  try {
    const [detail, scenario] = await Promise.all([
      loadDecisionDetail(context.decisionId, context.householdId, { signal: controller.signal }),
      modelDecisionScenario(context.decisionId, context.householdId, context.impactModel.scenarioInputs, { signal: controller.signal }),
    ]);
    if (controller !== state.decisionScenarioController || state.householdScenario?.decisionId !== context.decisionId || state.currentHouseholdId !== context.householdId) return;
    state.activeDecisionDetail = detail;
    state.activeDecisionScenario = scenario;
    state.householdScenario.implementationAmount = scenario.implementation.amount;
    state.householdScenario.impactModel = scenario.proposalModel;
    ...
    if (error.name !== "AbortError") showToast("Unable to restore decision controls. Reopen the household decision to try again.");
  } finally {
    if (controller === state.decisionScenarioController) { state.mandatePending = false; renderProposalTray(); }
  }
}
...
function proposalSolutionMarkup(proposal) {
  return `<div class="proposal-solutions-table-wrap"><table class="proposal-solutions-table"><thead><tr><th>Strategy</th><th>Portfolio role</th><th>Allocation</th><th>Weight</th><th>Product fee</th><th>Liquidity</th></tr></thead><tbody>${proposal.candidates.map((candidate) => { const fee = proposalCandidateFeeDisclosure(candidate); return `<tr><th><strong>${escapeHtml(candidate.name)}</strong><small>${escapeHtml(candidate.manager)} · ${escapeHtml(candidate.symbol || candidate.category)}</small></th><td>${escapeHtml(proposalCandidateRole(candidate))}</td><td>${formatWealthCurrency(candidate.amount)}</td><td>${proposal.totalAmount ? ((candidate.amount / proposal.totalAmount) * 100).toFixed(1) : 0}%</td><td>${escapeHtml(fee.label)}</td><td>${escapeHtml(candidate.liquidity || "See product materials")}</td></tr>`; }).join("")}</tbody></table></div>`;
}

function proposalReadinessMarkup(readiness) {
  if (readiness.ready) return `<div class="proposal-readiness-state ready"><span aria-hidden="true">✓</span><div><strong>Ready to finalize</strong><small>Required proposal details are complete.</small></div></div>`;
  return `<div class="proposal-readiness-state blocked"><span aria-hidden="true">!</span><div><strong>${readiness.blockers.length} ${readiness.blockers.length === 1 ? "item" : "items"} to resolve</strong><ul>${readiness.blockers.map((blocker) => `<li>${escapeHtml(blocker.label)}</li>`).join("")}</ul></div></div>`;
}

function syncProposalReadinessControls() {
  const readiness = getProposalReadiness(state.proposal);
  document.querySelectorAll("[data-proposal-generate]").forEach((button) => {
    button.disabled = !readiness.ready;
    button.title = readiness.ready ? "" : readiness.blockers.map((blocker) => blocker.label).join(" · ");
  });
  const status = document.querySelector("[data-proposal-readiness-status]");
  if (status) status.innerHTML = proposalReadinessMarkup(readiness);
}
...
function proposalSubtitle(proposal) {
  if (proposal.impactModel?.kind === "concentration") return `Reducing concentration in ${proposal.impactModel.sourceSymbol === "AAPL" ? "Apple" : proposal.impactModel.sourceName || proposal.impactModel.sourceSymbol || "a single position"}`;
...
function renderProposalBuilder() {
  ...
  const feeSummary = !feesComplete ? "Fee review required" : weightedFee > 0 ? `${weightedFee.toFixed(2)}% weighted product fee` : "No annual product fee";
  const costSummary = !feesComplete
    ? "Complete product-level fee data before finalizing this proposal."
    : weightedFee > 0
      ? `${weightedFee.toFixed(2)}% weighted annual product fee, approximately ${currency.format(annualProductCost)} per year on the proposed amount. Advisory, custody and transaction charges are additional where applicable.`
...
document.addEventListener("click", (event) => {
  ...
  if (event.target.closest("[data-proposal-back-results]")) returnToProposalSelection();
  if (event.target.closest("[data-open-decision-from-proposal]")) returnToProposalSelection();
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Escape" && !el("decisionStudio").hidden) { event.preventDefault(); closeDecisionStudio(); return; }
