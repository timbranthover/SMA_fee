...
import { investmentAllocationBucket } from "./proposal-impact.js";
import { createWealthRepository } from "./wealth-repository.js";
import { createWealthService } from "./wealth-service.js";

const ACTIVE_PLAN_STATUSES = new Set(["Plan drafted", "Ready for client"]);
...
function statusRank(status) {
  const order = { New: 5, Reviewing: 4, "Plan drafted": 3, "Ready for client": 2, Complete: 0 };
...
function implementationFor(record, overview, amount = 0) {
  if (record.implementationType === "diversified-us-equity") return {
    ...
    flags: [],
    risks: [],
    tags: [overview.household.name, "Compare ETFs and SMAs", "Reduce concentrated exposure"],
...
export function createDecisionService(dataset, { repository: repositoryOverride = null, wealthService: wealthServiceOverride = null, cacheLimits = {} } = {}) {
  ...
  function getDecisionDetail(householdId, decisionId) {
    ...
    if (record.kind === "concentration") {
      ...
      if (review) {
        const source = searchCatalog({ q: review.holding.symbol, pageSize: 1 }).items.find((item) => item.symbol === review.holding.symbol);
        model.sourceSymbol = review.holding.symbol;
        model.sourceName = review.holding.name;
        model.sourceBucket = source ? investmentAllocationBucket(source) : null;
        model.sourceValue = review.holding.value;
        facts.push(
        ...
        model.defaults = { targetWeight: review.targetWeight, stressDrop: 35, goalId: relatedGoal?.id || "", goalFunding: 0, redeployAmount: roundMoney(review.targetRelease) };
        model.affectedAccounts = review.accounts.map((account) => ({ name: account.name, value: account.value, gain: account.gain }));
  ...
  function modelDecisionScenario(householdId, decisionId, input = {}) {
    ...
    if (record.kind === "concentration") {
      ...
      const afterUsEquity = detail.model.sourceBucket ? Math.max(0, usEquity.amount - (detail.model.sourceBucket === "US equity" ? release : 0) + redeployAmount) : null;
    ...
    result.proposalModel = {
      version: 1, kind: record.kind, scenarioInputs: { ...result.inputs },
      financialAssets: overview.household.financialAssets,
      sourceSymbol: detail.model.sourceSymbol || "",
      sourceName: detail.model.sourceName || "",
      sourceValueBefore: detail.model.sourceValue || 0,
      sourceValueAfterSale: (detail.model.sourceValue || 0) - (result.economics.release || 0),
      cashBefore: result.before.cash,
      cashPolicyPct: 4,
      cashAfterFunding: result.after.cash + result.implementation.amount,
      usEquityBeforePct: pct(usEquity.amount, overview.household.financialAssets),
      usEquityAfterSale: record.kind === "concentration" && !detail.model.sourceBucket ? null
        : usEquity.amount - (detail.model.sourceBucket === "US equity" ? result.economics.release || 0 : 0),
      municipalBeforePct: result.before.allocationPct ?? null,
      estimatedRealizedGain: result.economics.realizedGain ?? null,
    };
  ...
  function getMeetingBrief(householdId) {
    ...
    const urgency = { Priority: 0, "Time sensitive": 1, Review: 2, Opportunity: 3 };
    const openDecisions = summary.decisions.filter((decision) => !CLOSED_STATUSES.has(decision.status))
      .sort((left, right) => (urgency[left.priority] ?? 4) - (urgency[right.priority] ?? 4) || left.title.localeCompare(right.title));
    const plans = openDecisions.filter((decision) => ACTIVE_PLAN_STATUSES.has(decision.status));
    const listedInsights = new Set(openDecisions.map((decision) => decision.sourceInsightId));
    const upcoming = overview.insights.filter((insight) => insight.severity === "Upcoming" && !listedInsights.has(insight.id));
    const changes = [
