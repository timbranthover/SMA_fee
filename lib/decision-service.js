...
function implementationFor(record, overview, amount = 0) {
  ...
  if (record.implementationType === "municipal-income") return {
    ...
    category: "Fixed Income",
    query: overview.household.location === "New York" ? "New York municipal income" : "municipal income",
    flags: ["Tax-Aware"],
    risks: ["Conservative"],
  ...
  function getDecisionDetail(householdId, decisionId) {
    ...
    if (record.kind === "concentration") {
      ...
      if (review) {
        ...
        model.defaults = { targetWeight: review.targetWeight, stressDrop: 35, goalId: relatedGoal?.id || "", goalFunding: 0, redeployAmount: roundMoney(Math.min(review.targetRelease, 1_000_000)) };
