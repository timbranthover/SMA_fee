...
function searchable(item, ordinal) {
  return {
    ...item,
    _ordinal: ordinal,
    _searchFields: SEARCH_FIELDS.map(({ key }) => normalizeSearchText(key === "flags" ? item.flags.join(" ") : item[key])),
    _symbolKey: normalizeIdentifier(item.symbol),
    _identifierKey: normalizeIdentifier(item.identifier),
  };
}
...
function publicItem(item, match) {
  const { _ordinal, _searchFields, _symbolKey, _identifierKey, ...result } = item;
...
export function searchCatalog(input = {}) {
  ...
  const filtered = getSearchIndex().filter((item) => matchesFilters(item, options));
  ...
  const items = matched.slice(start, start + options.pageSize).map((item) => publicItem(item, matchMetadata.get(item.id)));
