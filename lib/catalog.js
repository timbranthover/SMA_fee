...
function marketSnapshotFor(item) {
  ...
  if (item.category === "Equities") {
    return {
      ...
      trend: trend(),
      keyA: field("Forward P/E", `${fixed(seed, 5, 12, 34)}×`),
      keyB: field("Dividend yield", `${fixed(seed, 6, .2, 3.1)}%`),
      asOf,
    };
  }
  ...
  if (["Mutual Funds", "ETFs"].includes(item.category)) {
    ...
    return {
      ...
      trend: trend(),
      keyA: field("30-day SEC yield", `${fixed(seed, 6, bondLike ? 3.1 : .7, bondLike ? 5.9 : 2.2)}%`),
      keyB: field("Expense ratio", item.fee === null ? "—" : `${item.fee.toFixed(2)}%`),
      asOf,
    };
  }
  ...
  if (item.category === "SMAs") {
    ...
    return {
      ...
      keyA: field("Minimum", formatMoney(item.minimum)),
      keyB: field("Manager fee", item.fee === null ? "—" : `${item.fee.toFixed(2)}%`),
  ...
  if (item.category === "Fixed Income") {
    ...
    return {
      ...
      keyA: field("Yield to worst", `${yieldValue.toFixed(2)}%`),
      keyB: field("Credit rating", rating),
      asOf,
    };
  }
  ...
  if (item.category === "Alternatives") {
    ...
    return {
      ...
      keyA: field("3Y annualized", `${threeYear.toFixed(1)}%`),
      keyB: field("Liquidity", item.liquidity.split("·")[0].trim()),
  ...
  if (item.category === "Structured") {
    ...
    return {
      ...
      keyA: field("Contingent coupon", `${fixed(seed, 3, 6, 12)}%`),
      keyB: field("Term", `${fixed(seed, 4, 12, 36, 0)} months`),
      asOf,
    };
  }
  ...
  if (item.category === "Managed Options") {
    ...
    return {
      ...
      trend: trend("1Y composite", annualReturn),
      keyA: field("Minimum", formatMoney(item.minimum)),
      keyB: field("Annual fee", item.fee === null ? "—" : `${item.fee.toFixed(2)}%`),
      asOf: "Through Aug 21, 2026",
    };
  }
  ...
  if (item.category === "Annuities") {
    ...
    return {
      ...
      keyA: field("Guarantee period", `${fixed(seed, 3, 3, 10, 0)} years`),
      keyB: field("Annual fee", item.fee === null ? "—" : `${item.fee.toFixed(2)}%`),
      asOf,
    };
  }
  ...
  return {
    ...
    trend: trend(),
    keyA: field("1Y return", `${annualReturn >= 0 ? "+" : ""}${annualReturn.toFixed(1)}%`),
    keyB: field("Custody fee", item.fee === null ? "—" : `${item.fee.toFixed(2)}%`),
    asOf,
  };
}
