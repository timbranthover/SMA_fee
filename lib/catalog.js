...
function queryTokens(query) {
  return [...new Set(normalizeSearchText(query).split(/\s+/).filter((token) => token.length > 1 && !QUERY_STOP_WORDS.has(token) && !/^\d+(?:\.\d+)?$/.test(token)))];
}
...
function publicItem(item, match) {
  const { _ordinal, _searchFields, ...result } = item;
  ...
  if (item.category === "SMAs") {
    return {
      ...
      keyFacts: [
        ...
        { label: "Customization", value: item.flags.includes("Direct Indexing") ? "Restrictions & tilts" : "Guideline-based" }, { label: "Model delivery", value: item.flags.includes("Model Enabled") ? "Enabled" : "Not enabled" },
      ],
      performance: commonPerformance,
      riskMetrics: commonRisk,
