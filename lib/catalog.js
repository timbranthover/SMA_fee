...
function optionsFromInput(input = {}) {
  ...
  const parsed = exactSmaNames.has(normalizeSearchText(input.q || ""))
    ? { filters: { category: "SMAs" }, interpreted: [] }
    : parseNaturalLanguage(input.q || "");
  ...
  const feeMaximum = Number.isFinite(input.maxFee) ? input.maxFee : inferredFeeSupported ? parsed.filters.maxFee : undefined;
  ...
  return {
    ...
    interpreted: inferredFeeSupported ? parsed.interpreted : parsed.interpreted.filter((label) => !label.startsWith("Fee ≤")),
