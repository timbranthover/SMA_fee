import { illustrativeSMA, SMA_DEMO_MANAGERS, SMA_DEMO_STRATEGIES } from "./sma-universe.js";
...
const base = (item) => {
  ...
  if (record.manager?.startsWith("Illustrative ")) record.manager = record.manager.slice("Illustrative ".length);
  if (record.description?.startsWith("Illustrative ")) record.description = record.description.slice("Illustrative ".length);
...
export const CURATED = [
  ...
  base({ id: "ann-fixed", category: "Annuities", type: "Fixed annuity", name: "Secure Term Choice 5-Year", symbol: "STC5", manager: "Heritage Life", assetClass: "Annuity", objective: "Guaranteed accumulation", risk: "Conservative", minimum: 10000, fee: 0, perf1: 4.55, perf3: null, aum: "$11B carrier assets", flags: ["Research Updated"], description: "Fixed deferred annuity with a five-year guaranteed interest period, subject to carrier claims-paying ability.", benchmark: "5-Year Treasury", inception: "2026", liquidity: "Annual free-withdrawal limit" }),
  base({ id: "ann-rila", category: "Annuities", type: "Registered index-linked annuity", name: "Market Link Select RILA", symbol: "MLSR", manager: "Heritage Life", assetClass: "Annuity", objective: "Buffered market growth", risk: "Moderate", minimum: 25000, fee: 1.10, perf1: null, perf3: null, aum: "$18B carrier assets", flags: ["New to Shelf"], description: "RILA offering index-linked growth potential with selected downside buffer options.", benchmark: "S&P 500", inception: "2026", liquidity: "Surrender schedule applies", status: "New" }),
...
const pools = {
  ...
  SMAs: {
    ...
    names: SMA_DEMO_STRATEGIES.map((strategy) => strategy.name),
    managers: SMA_DEMO_MANAGERS,
  ...
  Alternatives: {
    ...
    managers: ["Global Partners", "Alternative Credit Management", "Private Markets Group", "Global Infrastructure Partners"],
  ...
  Annuities: {
    ...
    managers: ["Heritage Life", "National Insurance Carrier", "Premier Life & Annuity"],
    assets: ["Annuity"],
...
const FIXED_INCOME_PROFILES = Object.freeze([
  Object.freeze({ type: "US Treasury", name: "US Treasury Note", manager: "United States Treasury", assetClass: "US Government", objective: "Income / capital preservation", benchmark: "US Treasury Index", liquidity: "Intraday" }),
  Object.freeze({ type: "Municipal bond", name: "General Obligation Revenue Bond", manager: "Municipal Issuer", assetClass: "Municipal Bond", objective: "Tax-exempt income", benchmark: "Municipal Bond Index", liquidity: "Secondary market" }),
  Object.freeze({ type: "Corporate bond", name: "Senior Unsecured Corporate Note", manager: "Investment Grade Issuer", assetClass: "Investment Grade Corporate", objective: "Income", benchmark: "US Corporate Bond Index", liquidity: "Secondary market" }),
  Object.freeze({ type: "Agency bond", name: "Federal Agency Note", manager: "Federal Agency", assetClass: "Agency", objective: "Income / capital preservation", benchmark: "US Agency Bond Index", liquidity: "Secondary market" }),
  Object.freeze({ type: "Certificate of deposit", name: "Brokered Certificate of Deposit", manager: "US Depository Institution", assetClass: "Cash & Equivalents", objective: "Capital preservation / income", benchmark: "3-Month US Treasury Bill", liquidity: "Secondary market" }),
]);
...
let currentEtfRiskCountsCache = null;
function currentEtfRiskCounts() {
  if (!currentEtfRiskCountsCache) currentEtfRiskCountsCache = RISKS.map((risk) => getCategorySearchIndex("ETFs").filter((item) => item.risk === risk).length);
  return currentEtfRiskCountsCache;
}

function categoryFacets(category) {
  if (category === "SMAs") return currentSmaFacets();
  const counts = CATEGORY_FACET_COUNTS[category];
  const risks = category === "ETFs" ? currentEtfRiskCounts() : counts.risks;
  return {
    ...
    risks: Object.fromEntries(RISKS.map((name, index) => [name, risks[index]])),
    statuses: Object.fromEntries(STATUSES.map((name, index) => [name, counts.statuses[index]])),
  };
}

let currentSmaFacetsCache = null;
function currentSmaFacets() {
  return currentSmaFacetsCache ||= buildFacets(getCategorySearchIndex("SMAs"));
}

let currentDefaultFacetsCache = null;
function currentDefaultFacets() {
  if (currentDefaultFacetsCache) return currentDefaultFacetsCache;
  const previousEtfRisks = CATEGORY_FACET_COUNTS.ETFs.risks;
  const currentEtfRisks = currentEtfRiskCounts();
  const smas = currentSmaFacets();
  currentDefaultFacetsCache = {
    ...DEFAULT_FACETS,
    flags: Object.fromEntries(allFlagNames.map((flag, index) => [flag, DEFAULT_FACETS.flags[flag] - CATEGORY_FACET_COUNTS.SMAs.flags[index] + smas.flags[flag]])),
    risks: Object.fromEntries(RISKS.map((risk, index) => [risk, DEFAULT_FACETS.risks[risk] - previousEtfRisks[index] + currentEtfRisks[index] - CATEGORY_FACET_COUNTS.SMAs.risks[index] + smas.risks[risk]])),
  };
  return currentDefaultFacetsCache;
}
...
export function parseNaturalLanguage(query, explicitCategory = "All") {
  const q = String(query).toLowerCase();
  let residual = q;
  const interpreted = [];
  const filters = {};
  const segments = [];
  const consume = (pattern) => { residual = residual.replace(pattern, " "); };
  const categoryIntents = [
    [/\betfs?\b/g, "ETFs", "ETFs"],
    [/\bsmas?\b|separately managed/g, "SMAs", "SMAs"],
    [/mutual funds?|\bfunds?\b/g, "Mutual Funds", "Mutual funds"],
    [/\bmunicipal\b|\bmuni\b/g, "Fixed Income", "Municipal bonds", "municipal"],
    [/fixed income|\bbonds?\b/g, "Fixed Income", "Fixed income", "bond"],
    [/\btreasury\b|\btreasuries\b/g, "Fixed Income", "Treasuries", "treasury"],
    [/private credit|private equity|\bhedge\b|\balternatives?\b/g, "Alternatives", "Alternatives"],
    [/\bstructured\b|buffered note|\bcontingent\b/g, "Structured", "Structured solutions"],
    [/managed options?|covered call|options strategy/g, "Managed Options", "Managed options"],
    [/\bannuit(?:y|ies)\b/g, "Annuities", "Annuities"],
    [/precious metals?|\bgold\b|\bsilver\b/g, "Precious Metals", "Precious metals"],
    [/\bstocks?\b|\bequity\b|\bequities\b/g, "Equities", "Equities"],
  ];
  const vehicle = categoryIntents.find(([pattern, category]) => category !== "Fixed Income" && pattern.test(q));
  const adopted = vehicle || categoryIntents.find(([pattern]) => pattern.test(q));
  if (adopted && (explicitCategory === "All" || explicitCategory === adopted[1])) {
    filters.category = adopted[1];
    interpreted.push(adopted[2]);
    consume(adopted[0]);
    if (adopted[3]) segments.push(adopted[3]);
  }

  const flags = [];
  for (const [pattern, flag, label] of [
    [/\bsustainable\b|\bimpact\b|\besg\b/g, "Sustainable", "Sustainable"],
    [/tax[- ]aware|tax efficient|tax managed|tax-exempt/g, "Tax-Aware", "Tax-aware"],
    [/direct indexing?/g, "Direct Indexing", "Direct indexing"],
    [/model (?:delivery|delivered|enabled)/g, "Model Delivered", "Model delivered"],
    [/\bcio\b|house view/g, "CIO House View", "CIO house view"],
  ]) if (pattern.test(q)) { flags.push(flag); interpreted.push(label); consume(pattern); }
  if (flags.length) filters.flags = flags;
  for (const [pattern, risk, label] of [[/\bconservative\b|low risk/g, "Conservative", "Conservative risk"], [/\bmoderate\b/g, "Moderate", "Moderate risk"], [/\baggressive\b|high risk/g, "High", "High risk"]]) {
    if (pattern.test(q)) { filters.risks = [risk]; interpreted.push(label); consume(pattern); break; }
  }
  const bps = q.match(/(?:under|below|less than)\s+(\d+)\s*bps?/);
  const percent = q.match(/(?:under|below|less than)\s+(\d+(?:\.\d+)?)\s*%/);
  if (bps) { filters.maxFee = Number(bps[1]) / 100; interpreted.push(`Fee ≤ ${bps[1]} bps`); consume(/(?:under|below|less than)\s+\d+\s*bps?/g); }
  else if (percent) { filters.maxFee = Number(percent[1]); interpreted.push(`Fee ≤ ${percent[1]}%`); consume(/(?:under|below|less than)\s+\d+(?:\.\d+)?\s*%/g); }
  if (/\blow[- ]fee\b|\blow[- ]cost\b/.test(q) && filters.maxFee === undefined) { filters.lowFee = true; interpreted.push("Lowest-fee quartile"); }
  consume(/\blow[- ](?:fee|cost)\b/g);
  if (/\blarge[- ]cap\b/.test(q)) { segments.push("large-cap"); interpreted.push("Large cap"); consume(/\blarge[- ]cap\b/g); }
  if (/new york|\bny\b/.test(q)) { filters.location = "New York"; interpreted.push("New York"); consume(/new york|\bny\b/g); }
  consume(/\b(?:fee|fees|cost|cap|under|below|less|than|low)\b/g);
  return { filters, interpreted, segments, residual: residual.replace(/\s+/g, " ").trim() };
}

const QUERY_STOP_WORDS = new Set(["with", "client", "investment", "investments", "looking", "need", "solution", "solutions", "aligned", "building", "blocks", "the", "and", "for", "from", "that", "this"]);
...
const exactSmaNames = new Set([
  ...CURATED.filter((item) => item.category === "SMAs").map((item) => normalizeSearchText(item.name)),
  ...Array.from({ length: CATEGORY_COUNTS.SMAs - CURATED.filter((item) => item.category === "SMAs").length }, (_, index) => normalizeSearchText(illustrativeSMA(index).name)),
]);
...
function matchesBaseFilters(item, options) {
  ...
  const text = `${item.assetClass} ${item.type} ${item.name} ${item.objective}`.toLowerCase();
  for (const segment of options.segments || []) {
    if (segment === "municipal" && !/\bmuni(?:cipal)?\b|tax[- ]exempt/.test(text)) return false;
    if (segment === "bond" && !/\bbond\b|fixed income|treasury|municipal|government|corporate debt|aggregate/.test(text)) return false;
    if (segment === "treasury" && !/treasury|t-bill|us government/.test(text)) return false;
    if (segment === "large-cap" && !/large|s&p 500|sp500|russell 1000/.test(text)) return false;
  }
  return true;
}
...
function illustrativeETFClassification(name, ticker) {
  const text = `${ticker} ${name}`.toLowerCase();
  const matches = (pattern) => pattern.test(text);
  if (matches(/\b(?:2x|3x|ultra|inverse|bear|bull)\b|\bdaily\b.*\b(?:leveraged|bull|bear)\b|\bshort\b.*\b(?:s&p|nasdaq|dow|russell|equity|bitcoin|ether)\b/))
    return { assetClass: "Leveraged / Inverse", objective: "Tactical exposure", risk: "High", benchmark: "Reference Index" };
  if (matches(/\b(?:bitcoin|ether|ethereum|crypto|digital asset)\b/))
    return { assetClass: "Digital Assets", objective: "Diversification", risk: "High", benchmark: "Digital Asset Reference" };
  if (matches(/\b(?:gold|silver|platinum|palladium|commodity|commodities|oil|natural gas|uranium|copper|agriculture)\b/))
    return { assetClass: "Commodities", objective: "Diversification", risk: "High", benchmark: "Commodity Reference" };
  if (matches(/\b(?:municipal|muni)\b/))
    return { assetClass: "Municipal Bond", objective: "Tax-exempt income", risk: "Conservative", benchmark: "Municipal Bond Index" };
  if (matches(/\b(?:treasury|t-bill|government bond)\b/))
    return { assetClass: matches(/\b(?:short|ultrashort|0-3|0 to 3|1-3|1 to 3|floating rate)\b/) ? "Short Government" : "US Government", objective: "Income", risk: "Conservative", benchmark: "US Treasury Index" };
  if (matches(/\b(?:high yield|junk bond)\b/))
    return { assetClass: "High Yield Bond", objective: "Income", risk: "High", benchmark: "High Yield Bond Index" };
  if (matches(/\b(?:corporate bond|investment grade|credit bond)\b/))
    return { assetClass: "Investment Grade Corporate", objective: "Income", risk: "Moderate", benchmark: "US Corporate Bond Index" };
  if (matches(/\b(?:bond|fixed income|aggregate|mortgage-backed|mbs|clo|senior loan|bank loan)\b/))
    return { assetClass: "Core Bond", objective: "Income", risk: "Conservative", benchmark: "Bloomberg US Aggregate" };
  if (matches(/\b(?:preferred|preferred securities)\b/))
    return { assetClass: "Preferred Securities", objective: "Income", risk: "Moderate", benchmark: "Preferred Securities Index" };
  if (matches(/\b(?:real estate|reit)\b/))
    return { assetClass: "Real Estate Equity", objective: "Income", risk: "Moderate", benchmark: "Real Estate Index" };
  if (matches(/\b(?:emerging markets|emerging market)\b/))
    return { assetClass: "Emerging Markets Equity", objective: "Diversification", risk: "High", benchmark: "Emerging Markets Index" };
  if (matches(/\b(?:international|developed markets|eafe|ex[- ]?u\.?s\.?|europe|japan|china|india|asia|latin america)\b/))
    return { assetClass: "International Equity", objective: "Diversification", risk: matches(/\b(?:china|india|latin america)\b/) ? "High" : "Moderate", benchmark: "International Equity Index" };
  if (matches(/\b(?:global|world|all-country|acwi)\b/))
    return { assetClass: "Global Equity", objective: "Diversification", risk: "Moderate", benchmark: "Global Equity Index" };
  if (matches(/\bsmall[- ]?cap\b|\brussell 2000\b/))
    return { assetClass: "US Small Cap Equity", objective: matches(/\bvalue\b/) ? "Core equity" : "Capital appreciation", risk: "High", benchmark: "US Small Cap Index" };
  if (matches(/\bmid[- ]?cap\b/))
    return { assetClass: "US Mid Cap Equity", objective: "Capital appreciation", risk: "Moderate", benchmark: "US Mid Cap Index" };
  if (matches(/\b(?:nasdaq[- ]?100|qqq)\b/))
    return { assetClass: "US Large Growth", objective: "Capital appreciation", risk: "Moderate", benchmark: "Nasdaq-100" };
  if (matches(/\bs&p\s*500\b|\b500 index\b/))
    return { assetClass: "US Large Blend", objective: "Core equity", risk: "Moderate", benchmark: "S&P 500" };
  if (matches(/\b(?:total stock market|total market|broad market)\b/))
    return { assetClass: "US Broad Market Equity", objective: "Core equity", risk: "Moderate", benchmark: "Broad US Equity Market" };
  if (matches(/\b(?:dividend|equity premium income|covered call|option income)\b/))
    return { assetClass: "US Large Blend", objective: "Income", risk: "Moderate", benchmark: "Broad US Equity Market" };
  if (matches(/\b(?:technology|semiconductor|software|cybersecurity|artificial intelligence|robotics|biotech|health care|healthcare|financial|energy|utilities|industrials|consumer|materials|communication services)\b/))
    return { assetClass: "Sector / Thematic Equity", objective: "Capital appreciation", risk: "High", benchmark: "Sector Equity Index" };
  if (matches(/\bvalue\b/))
    return { assetClass: "US Large Value", objective: "Core equity", risk: "Moderate", benchmark: "US Value Index" };
  if (matches(/\bgrowth\b/))
    return { assetClass: "US Large Growth", objective: "Capital appreciation", risk: "Moderate", benchmark: "US Growth Index" };
  return { assetClass: "US Large Blend", objective: "Core equity", risk: "Moderate", benchmark: "Broad US Equity Market" };
}

const EQUITY_MARKET_SIZE_OVERRIDES = new Map([
...
function makeSynthetic(category, index) {
  ...
  const fixedIncomeProfile = category === "Fixed Income" ? FIXED_INCOME_PROFILES[seed % FIXED_INCOME_PROFILES.length] : null;
  ...
  const etfClassification = etfReference ? illustrativeETFClassification(etfName, etfTicker) : null;
  const longTailEquityIndex = Math.max(0, index - EQUITY_UNIVERSE.length);
  let type = equityReference ? referencedEquityType(referenceName, referenceTicker) : etfReference ? (/\bETN\b/i.test(etfName) ? "ETP" : /\bactive\b/i.test(etfName) ? "Active ETF" : "ETF") : fixedIncomeProfile?.type || pick(pool.types, seed, 1);
  let name = equityReference ? referenceName : etfReference ? etfTicker === "VTI" ? "Vanguard Total Stock Market ETF" : etfName : category === "Equities" ? illustrativeEquityName(longTailEquityIndex) : fixedIncomeProfile?.name || pick(pool.names, seed, 2);
  ...
  let minimum = pick(pool.minimums, seed, 6);
  let fee = pick(pool.fees, seed, 7);
  ...
  let symbol = referenceTicker || etfTicker || generatedSymbol;
  const id = equityReference ? `eq-sec-${index}` : etfReference ? `etf-listed-${index}` : `syn-${slugCategory(category)}-${index}`;
  let manager = equityReference ? `Public equity · ${referenceExchange}` : etfReference ? etfIssuer : pick(pool.managers, seed, 8);
  ...
  let benchmark = equityReference ? "Broad Equity Market" : pick(["S&P 500", "Russell 1000", "Bloomberg US Aggregate", "Custom Strategy Benchmark"], seed, 11);
  let liquidity = category === "Alternatives" ? "Quarterly or less frequent" : category === "Fixed Income" ? "Secondary market" : "Daily";
  if (etfClassification) {
    assetClass = etfClassification.assetClass;
    objective = etfClassification.objective;
    risk = etfClassification.risk;
    benchmark = etfClassification.benchmark;
  }
  if (category === "SMAs") {
    const strategy = illustrativeSMA(index);
    ({ name, manager, type, symbol, assetClass, objective, benchmark, risk, minimum, fee } = strategy);
    const retainedFlags = flags.filter((flag) => !["Tax-Aware", "Direct Indexing", "Sustainable"].includes(flag));
    flags.splice(0, flags.length, ...retainedFlags, ...strategy.flags);
  }
  if (fixedIncomeProfile) {
    ({ manager, assetClass, objective, benchmark, liquidity } = fixedIncomeProfile);
  }
  return base({
    id,
    category,
    ...
    description: equityReference
      ? `${name} provides access to ${assetClass.toLowerCase()} for investors seeking ${objective.toLowerCase()}. Review valuation, liquidity and portfolio fit before implementation.`
      : etfReference
        ? `${name} offers exchange-traded access to ${assetClass.toLowerCase()} with a ${objective.toLowerCase()} objective. Review the fund's current materials, costs and underlying exposures.`
        : `${name} seeks ${objective.toLowerCase()} through ${assetClass.toLowerCase()}. Review the mandate, risks and implementation terms before use.`,
    benchmark,
...
const feeQuartiles = new Map();
function bottomFeeQuartile(category) {
  if (!feeQuartiles.has(category)) {
    const fees = getSearchIndex(category).filter((item) => item.fee !== null && item.fee !== undefined).map((item) => Number(item.fee)).filter((fee) => Number.isFinite(fee) && fee >= 0).sort((a, b) => a - b);
    feeQuartiles.set(category, fees.length ? fees[Math.floor((fees.length - 1) * .25)] : null);
  }
  return feeQuartiles.get(category);
}
...
function optionsFromInput(input = {}) {
  ...
  const parsed = exactSmaNames.has(normalizeSearchText(input.q || ""))
    ? { filters: { category: "SMAs" }, interpreted: [], residual: input.q || "", segments: [] }
    : parseNaturalLanguage(input.q || "", input.category || "All");
  const q = input.q?.trim() || "";
  const excluded = new Set(input.excludedQueryFilters || []);
  const category = input.category && input.category !== "All" ? input.category : input.suppressInference ? "All" : parsed.filters.category || "All";
  const inferred = input.suppressInference ? { flags: [], risks: [], ranges: {} } : {
    flags: (parsed.filters.flags || []).filter((flag) => !excluded.has(`flag:${flag}`)),
    risks: (parsed.filters.risks || []).filter((risk) => !excluded.has(`risk:${risk}`)),
    ranges: {},
  };
  ...
  const inferredFeeSupported = isRangeAllowed("fee", category);
  const feeMaximum = Number.isFinite(input.maxFee) ? input.maxFee : inferredFeeSupported && !input.suppressInference ? parsed.filters.maxFee ?? (parsed.filters.lowFee ? bottomFeeQuartile(category) : undefined) : undefined;
  if (Number.isFinite(feeMaximum) && rawRanges.fee?.max === undefined && !excluded.has("range:fee")) {
    rawRanges.fee = { ...(rawRanges.fee || {}), max: feeMaximum };
    if (!Number.isFinite(input.maxFee)) inferred.ranges.fee = { max: feeMaximum };
  }
  ...
  return {
    q,
    category,
    flags: [...new Set([...(input.flags || []), ...inferred.flags])],
    risks: [...new Set([...(input.risks || []), ...inferred.risks])],
    statuses: input.statuses || [],
    ranges,
    maxMinimum: ranges.minimum?.max,
    maxFee: ranges.fee?.max,
    location: input.location || (input.suppressInference ? undefined : parsed.filters.location),
    sort,
    cursor: Math.max(0, Number(input.cursor) || 0),
    pageSize: Math.min(25, Math.max(1, Number(input.pageSize) || 25)),
    interpreted: input.suppressInference ? [] : inferredFeeSupported ? parsed.interpreted : parsed.interpreted.filter((label) => !label.startsWith("Fee ≤") && label !== "Lowest-fee quartile"),
    queryFilters: inferred,
    segments: input.suppressInference ? [] : parsed.segments,
    hasQueryIntent: !input.suppressInference && Boolean(parsed.filters.category || parsed.filters.flags?.length || parsed.filters.risks?.length || parsed.filters.lowFee || parsed.filters.maxFee !== undefined || parsed.segments.length),
    tokens: queryTokens(input.suppressInference ? q : parsed.residual),
...
function categoricalFacetsFor(items, options) {
  ...
  return options.category === "All" ? currentDefaultFacets() : categoryFacets(options.category);
}
...
function publicItem(item, match) {
  ...
  if (item.category === "SMAs") result.benchmark = item.benchmark;
  if (item.category === "SMAs" || item.status !== "Available") result.status = item.status;
...
function marketSnapshotFor(item) {
  ...
  if (item.category === "Annuities") {
    ...
    return {
      ...
      trend: trend("Modeled growth", rate),
...
function searchResponse({ options, matched, matchMetadata = new Map(), facets, searchMode, started, total = matched.length, pageReady = false, notice = null }) {
  const start = Math.min(options.cursor, total);
  const page = pageReady ? matched : matched.slice(start, start + options.pageSize);
  const items = page.map((item) => {
    const result = publicItem(item, matchMetadata.get(item.id));
    if (options.sort.startsWith("primary-") && ["Equities", "ETFs"].includes(item.category)) result.sortPrice = marketMetricNumber(item, "primary");
    return result;
  });
  ...
  return {
    ...
    appliedFilters: { flags: options.flags, risks: options.risks, query: options.queryFilters },
...
export function searchCatalog(input = {}) {
  ...
  if (exactSmaNames.has(options.normalizedQuery)) {
    const rangeSource = getSearchIndex("SMAs").filter((item) => normalizeSearchText(item.name) === options.normalizedQuery && matchesBaseFilters(item, options));
    const matched = rangeSource.filter((item) => matchesRangeFilters(item, options));
    const matchMetadata = new Map(matched.map((item) => [item.id, { score: 1250, matchReason: "Exact product name match", matchMode: "strict" }]));
  ...
  if (isInitialShelf(options)) {
    ...
    return searchResponse({ options, matched, facets: currentDefaultFacets(), searchMode: "filters", started, total: UNIVERSE_SIZE, pageReady: true });
  }
  ...
  if (options.tokens.length && !candidateCategories.length) {
    ...
    let matched = rangeSource.filter((item) => matchesRangeFilters(item, options));
    let notice = null;
    if (!matched.length && options.hasQueryIntent && filtered.some((item) => matchesRangeFilters(item, options))) {
      matched = filtered.filter((item) => matchesRangeFilters(item, options));
      searchMode = "filter-fallback";
      notice = "No exact wording match. Showing investments matching the interpreted filters.";
    }
    sortItems(matched, options, matchMetadata);
    const facets = categoricalFacetsFor(matched, options);
    facets.ranges = rangeFacetsFor(rangeSource, options);
    return searchResponse({ options, matched, matchMetadata, facets, searchMode, started, notice });
  }

  if (!options.tokens.length && !options.hasQueryIntent && options.normalizedQuery && options.identifierQuery && !/\d/.test(options.identifierQuery)) {
  ...
  } else if (options.normalizedQuery && options.identifierQuery && !options.hasQueryIntent) rangeSource = [];
  let matched = rangeSource.filter((item) => matchesRangeFilters(item, options));
  let notice = null;
  if (!matched.length && options.tokens.length && options.hasQueryIntent && filtered.some((item) => matchesRangeFilters(item, options))) {
    matched = filtered.filter((item) => matchesRangeFilters(item, options));
    searchMode = "filter-fallback";
    notice = "No exact wording match. Showing investments matching the interpreted filters.";
  }
...
function controlsFor(item, seed) {
  ...
  const shelf = item.status === "Limited"
    ...
    : item.status === "New"
      ...
      : { label: "Available", tone: "approved", detail: "Open on the product shelf", effective: "Aug 1, 2026" };
  ...
  const sources = {
    Equities: "Market Data Service",
    "Mutual Funds": "Fund Data Feed",
    ETFs: "Fund Data Feed",
  ...
  const data = {
    ...
    source: item.category === "ETFs" && item.referenceSource ? "Nasdaq Trader ETF reference + modeled analytics" : item.category === "Equities" && item.referenceSource ? "SEC issuer reference + modeled analytics" : sources[item.category] || "UPS Product Master",
...
function profileFor(item, seed, performanceSeries) {
  ...
  const commonPerformance = {
    ...
    benchmarkSeries,
    rows: [
      { period: "1 year", investment: oneYear, benchmark: Number((oneYear + Number(fixed(seed, 1, -0.9, 0.9))).toFixed(1)) },
  ...
  const commonRisk = [
    ...
    { label: "Maximum drawdown", value: `-${fixed(seed, 11, 6.5, 24.0)}%`, context: "Modeled trailing five years" },
  ];
  ...
  const research = {
    ...
    bullets: [
      ...
      `${item.status === "Available" ? "Currently available on the shelf." : `Current shelf status: ${item.status}.`}`,
    ],
  };
  ...
  if (item.category === "Equities") {
    ...
    return {
      ...
      keyFacts: [
        { label: "Market capitalization", value: item.aum }, { label: item.referenceSource ? "Primary market" : "Sector", value: item.referenceSource ? listing : sector },
      ...
      composition: { title: "Fundamentals", subtitle: "Operating profile", breakdown: [
  ...
  if (["Mutual Funds", "ETFs"].includes(item.category)) {
    ...
    return {
      ...
      performance: commonPerformance,
      riskMetrics: [...commonRisk, { label: "Fund category", value: item.assetClass, context: "Workspace classification" }],
      composition: { title: bondLike ? "Portfolio exposure" : "Holdings & exposure", subtitle: `Modeled allocations as of ${item.asOf}`, breakdown: bondLike
      ...
      fees: [{ label: "Management fee", value: item.fee === null ? "—" : `${item.fee.toFixed(2)}%` }, { label: "12b-1 / distribution", value: item.type.includes("Institutional") || isEtf ? "0.00%" : "0.10%" }, { label: "Transaction fee", value: "See trading terms" }, { label: "Gross expense ratio", value: item.fee === null ? "—" : `${item.fee.toFixed(2)}%` }],
  ...
  if (item.category === "Fixed Income") {
    ...
    return {
      quote: { label: "Clean price", value: `$${price}`, change: `${yieldValue}% yield to worst`, changeTone: "neutral", secondaryLabel: "Accrued interest", secondaryValue: `$${fixed(seed, 5, .18, 2.42, 2)}`, asOf },
      keyFacts: [
      ...
      performance: { ...commonPerformance, title: "Yield & return history", subtitle: "Modeled total-return comparison", rows: commonPerformance.rows.slice(0, 3) },
      riskMetrics: [{ label: "Interest-rate risk", value: `${fixed(seed, 7, 1.2, 11.8)} duration`, context: "Effective duration" }, { label: "Credit quality", value: item.type.includes("Treasury") ? "U.S. government" : "Investment grade", context: "Modeled composite rating" }, { label: "Call status", value: seed % 2 ? "Non-callable" : "Callable", context: seed % 2 ? "—" : `First call ${2028 + seed % 5}` }, { label: "Price sensitivity", value: `${fixed(seed, 8, 1.0, 10.4)}%`, context: "Approx. move for +100 bps" }],
  ...
  return {
    quote: { label: profile.quoteLabel, value: profile.quoteValue, change: profile.change, changeTone: dayChange >= 0 ? "positive" : "neutral", secondaryLabel: "Data basis", secondaryValue: "Latest modeled valuation", asOf: item.asOf },
    keyFacts: profile.facts.map(([label, value]) => ({ label, value })),
    performance: commonPerformance,
    riskMetrics: commonRisk,
    composition: { title: item.category === "Structured" ? "Payoff profile" : "Exposure & structure", subtitle: "Modeled characteristics", breakdown: profile.composition.map(([label, value]) => ({ label, value })), holdings: profile.holdings },
...
function resolveInvestmentRecord(id) {
  ...
  if (!item) {
    const etfIndex = etfReferenceIndexByTicker.get(normalizeIdentifier(requested));
    if (etfIndex !== undefined) item = makeSynthetic("ETFs", etfIndex);
  }
  if (!item && requested.startsWith("etf-listed-")) {
    const etfIndex = Number(requested.slice("etf-listed-".length));
    if (Number.isInteger(etfIndex) && etfIndex >= 0 && etfIndex < etfReferenceTail.length) item = makeSynthetic("ETFs", etfIndex);
  }
  if (!item) {
