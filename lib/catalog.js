...
function makeSynthetic(category, index) {
  ...
  let name = equityReference ? referenceName : etfReference ? etfName : category === "Equities" ? illustrativeEquityName(longTailEquityIndex) : fixedIncomeProfile?.name || pick(pool.names, seed, 2);
  ...
  return base({
    ...
    description: equityReference
      ...
      : etfReference
        ? `${etfName} is included in the prototype ETF universe using current Nasdaq Trader ticker, fund-name and listing-exchange reference data. Asset-class, objective, risk and benchmark labels are illustrative classifications derived from the fund name; fee, AUM, research and product-control fields remain illustrative.`
        : `Illustrative ${objective.toLowerCase()} solution in ${assetClass.toLowerCase()}, managed by ${manager}. ${category === "SMAs" ? "Illustrative strategy, terms and performance; not an actual manager product." : "Designed to demonstrate realistic shelf data and governed product metadata."}`,
    benchmark,
...
function controlsFor(item, seed) {
  ...
  const data = {
    ...
    source: item.category === "ETFs" && item.referenceSource ? "Nasdaq Trader ETF reference + illustrative market data" : item.category === "Equities" && item.referenceSource ? "SEC issuer reference + illustrative market data" : sources[item.category] || "UPS Product Master",
