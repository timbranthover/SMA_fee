...
function generatedHousehold(index) {
  ...
  const insights = [];
  if (concentration) {
    insights.push({ id: `${id}-concentration`, kind: "concentration", householdId: id, severity: "Priority", tone: "red", title: `${primaryHolding.symbol} concentration is ${primaryHolding.householdWeightPct.toFixed(1)}%`, detail: `${moneyShort(primaryHolding.marketValue)} across ${positions.length || 1} accounts`, actionLabel: "Review", actionMetadata: { type: "concentration" } });
  }
  if (cashPct >= 10) insights.push({ id: `${id}-cash`, kind: "liquidity", householdId: id, severity: "Opportunity", tone: "green", title: `${moneyShort(cash)} available for investment`, detail: `${cashPct.toFixed(1)}% cash across the household`, actionLabel: "Explore", actionMetadata: { type: "investment-search", searchIntent: { source: "FROM LIQUIDITY REVIEW", title: "Explore cash alternatives", tags: [`${moneyShort(cash)} available`, riskProfile, "Daily liquidity"], category: "Fixed Income", query: "short duration cash management", flags: [], risks: ["Conservative"] } } });
  if (watchGoals.length) insights.push({ id: `${id}-goal-review`, kind: "goal", householdId: id, severity: "Planning", tone: "amber", title: `${watchGoals[0].name} needs review`, detail: `${Math.round(watchGoals[0].fundedAmount / watchGoals[0].targetAmount * 100)}% funded · ${watchGoals[0].nextReview}`, actionLabel: "Review", actionMetadata: { type: "goal", goalId: watchGoals[0].id } });
  if (seed % 7 === 0) {
    const callAmount = round(50_000 + (Math.floor(seed / 7) % 6) * 25_000, 5000);
    insights.push({ id: `${id}-capital-call`, kind: "obligation", householdId: id, severity: "Upcoming", tone: "amber", title: `${moneyShort(callAmount)} private-investment capital call`, detail: "Due within 30 days · funding source identified", actionLabel: "Review", actionMetadata: { type: "detail" }, details: { eyebrow: "UPCOMING OBLIGATION", summary: "Available household cash covers the upcoming obligation without requiring an investment sale.", rows: [["Funding source", accounts[0].name], ["Cash available", moneyShort(cash)], ["Call amount", moneyShort(callAmount)], ["Status", "Funding source identified"]] } });
  }
  if (seed % 5 === 0) insights.push({ id: `${id}-research-change`, kind: "research", householdId: id, severity: "Research", tone: "slate", title: "Two followed investments changed", detail: "Research and shelf updates since last review", actionLabel: "View", actionMetadata: { type: "detail" }, details: { eyebrow: "FOLLOWED INVESTMENTS", summary: "Research and shelf activity tied to investments followed in this relationship.", rows: [[holdingSnapshots[1].name, "Research review completed"], [holdingSnapshots[2].name, "Data refreshed"]] } });
  if (!insights.length) insights.push({ id: `${id}-planning-current`, kind: "planning", householdId: id, severity: "Current", tone: "blue", title: "Household plan remains on track", detail: "No material exceptions since the last review", actionLabel: "View", actionMetadata: { type: "detail" } });
  ...
  const concentrationPolicies = concentration ? [{
    ...
    searchIntent: { source: "FROM CONCENTRATION REVIEW", title: "Explore diversification options", tags: [name, "Tax-aware implementation", "Reduce concentrated exposure"], category: "SMAs", query: "", flags: ["Tax-Aware", "Direct Indexing"], risks: [/conservative/i.test(riskProfile) ? "Conservative" : "Moderate"] },
  }] : [];
