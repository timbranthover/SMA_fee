...
import { ADVISOR_IDENTITY } from "./advisor-identity.js";

const ADVISOR_ID = ADVISOR_IDENTITY.id;
...
const HOUSEHOLD_EQUITY_POOL_LIMIT = 192;
...
const HOLDINGS = [...SCREENER_EQUITIES, ...DIVERSIFIERS];
...
function generatedHousehold(index) {
  ...
  const primaryHoldingKey = `${primaryHolding[0]}:${primaryHolding[1]}`;
  const otherHoldings = selectDistinct(HOLDINGS, seed, 6, 80, new Set([primaryHoldingKey]), (holding) => `${holding[0]}:${holding[1]}`);
  ...
  if (cashPct >= 9.5) {
    ...
    insights.push({
      ...
      actionLabel: "Explore",
      actionMetadata: { type: "investment-search" },
    });
  }
  ...
  if (watchGoals.length) {
    ...
    insights.push({
      ...
      detail: `${Math.round(goal.fundedAmount / goal.targetAmount * 100)}% funded · ${goal.nextReview} · ${moneyShort(goal.targetAmount)} target`,
      actionLabel: "Review",
  ...
  if (seed % 8 === 0) {
    ...
    const baseAmount = round(obligation.min + mixedIndex(seed, 115, obligation.maxSteps) * obligation.step, 5000);
    const amount = id === "household-038" ? baseAmount : baseAmount + (mixedIndex(seed, 135, 9) + 1) * 400;
    const dueDate = new Date(Date.UTC(2026, 9 + mixedIndex(seed, 136, 3), 4 + mixedIndex(seed, 137, 23)));
    const due = `Due ${dueDate.toLocaleDateString("en-US", { month: "short", day: "numeric", timeZone: "UTC" })}`;
    const sourceIdentified = mixedIndex(seed, 138, 10) >= 3;
    const sourceState = sourceIdentified ? "funding source identified" : "funding source not identified";
    const title = obligation.key === "trust-distribution" ? `${family} trust distribution`
      : obligation.key === "capital-call" ? `${pick(["Blackstone Private Credit", "Carlyle Credit", "Harborview Direct Lending"], seed, 139)} capital call`
      : obligation.key === "business-commitment" ? `${family} business investment commitment`
      : `${family} ${obligation.label}`;
    insights.push({
      ...
      title: `${amount.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })} ${title}`,
      detail: `${due} · ${sourceState}`,
      actionLabel: "Review",
      ...
      details: {
        ...
        rows: [[sourceIdentified ? "Funding source" : "Potential funding source", accounts[0].name], ["Cash available", moneyShort(cash)], ["Amount", amount.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })], ["Timing", due], ["Status", sourceState]],
      },
    });
  }
  ...
  if (heldAwayAccounts.length && seed % 11 === 0) {
  ...
  if (!insights.length) {
    insights.push({
      ...
      detail: `${family} accounts · last review ${pick(["May", "Jun", "Jul", "Aug"], seed, 125)} ${8 + mixedIndex(seed, 126, 20)}`,
      actionLabel: "View",
      actionMetadata: { type: "detail" },
    });
  }
