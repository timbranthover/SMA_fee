...
export function applyQuoteToSnapshot(snapshot, quote) {
  ...
  return {
    ...snapshot,
    reference: snapshot.reference || { primary: snapshot.primary, metrics: snapshot.metrics },
...
export function applyMetricsToSnapshot(snapshot, metrics, category) {
  ...
  if (metrics.perf1 !== null && metrics.perf1 !== undefined) {
    additions.perf1 = { label: "Trailing 1 year", value: signedPercent(metrics.perf1, 1) };
  ...
  return { ...snapshot, reference: snapshot.reference || { primary: snapshot.primary, metrics: snapshot.metrics }, metrics: { ...(snapshot.metrics || {}), ...additions }, live: { ...(snapshot.live || {}), ...live } };
}
