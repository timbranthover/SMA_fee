...
function annualizedReturn(points, years) {
  ...
  for (const point of points) {
    if (point.time <= cutoffSeconds) start = point;
    else break;
  }
...
async function getReturnMetrics(symbol, options = {}) {
  const key = symbolKey(symbol);
  if (!key) return null;
  ...
  try {
    ...
    return store(returnCache, key, { perf1, perf3, provider: "Yahoo Finance" });
  } catch {
    return null;
  }
}
...
export async function getLiveMetrics(items, options = {}) {
  ...
  const [quoteRows, returnRows] = await Promise.all([
    ...
    mapWithConcurrency(eligible, 8, async (item) => ({ item, returns: await getReturnMetrics(item.symbol, options) })),
  ]);
  const returnsById = new Map(returnRows.filter((row) => row.returns).map((row) => [row.item.id, row.returns]));
  const result = new Map();
  for (const item of eligible) {
    const row = quoteRows.get(symbolKey(item.symbol)) || {};
    const returns = returnsById.get(item.id) || {};
    const metrics = {
