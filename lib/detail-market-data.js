...
export function comparisonFee(item, snapshot, detail) {
  return item.category === "ETFs" ? (snapshot?.live?.expenseRatio ?? (detail?.id === item.id ? detail.fee : item.fee)) : item.fee;
}
...
export function applyMetricsToDetail(detail, metrics) {
  ...
  const performance = detail.profile?.performance
    ? {
        ...
        subtitle: perf1 !== null || perf3 !== null
          ? "Live 1Y total / 3Y annualized fund returns · benchmark and longer periods modeled"
  ...
  return {
    ...
    profile: {
      ...
      keyFacts,
      performance,
      fees: detail.category === "ETFs" && root.fee !== undefined
        ? (detail.profile?.fees || []).map((row) => ["Management fee", "Gross expense ratio"].includes(row.label) ? { ...row, value: percentText(root.fee) } : row)
        : detail.profile?.fees,
    },
  };
}
