...
const OPTIONAL_COLLECTIONS = Object.freeze(["decisions", "actionPlans", "actions"]);
...
export function createWealthRepository(dataset) {
  ...
  const optional = Object.fromEntries(OPTIONAL_COLLECTIONS.map((collection) => [collection, Array.isArray(dataset[collection]) ? dataset[collection] : EMPTY]));
  ...
  const decisionsById = buildIdIndex(optional.decisions, "decisions");
  const actionPlansById = buildIdIndex(optional.actionPlans, "actionPlans");
  const actionsById = buildIdIndex(optional.actions, "actions");
  ...
  assertForeignKey(optional.decisions, "householdId", householdsById, "decision");
  assertForeignKey(optional.decisions, "advisorId", advisorsById, "decision");
  assertForeignKey(optional.decisions, "signalId", insightsById, "decision");
  assertForeignKey(optional.actionPlans, "householdId", householdsById, "action plan");
  assertForeignKey(optional.actionPlans, "decisionId", decisionsById, "action plan");
  assertForeignKey(optional.actions, "householdId", householdsById, "action");
  assertForeignKey(optional.actions, "planId", actionPlansById, "action");
  ...
  for (const decision of optional.decisions) {
    ...
    const signal = insightsById.get(decision.signalId);
    assert(signal.householdId === decision.householdId, `decision ${decision.id} crosses household/signal boundaries`);
  }
  for (const plan of optional.actionPlans) {
    const decision = decisionsById.get(plan.decisionId);
    assert(decision.householdId === plan.householdId, `action plan ${plan.id} crosses decision/household boundaries`);
  }
  for (const action of optional.actions) {
    const plan = actionPlansById.get(action.planId);
    assert(plan.householdId === action.householdId, `action ${action.id} crosses plan/household boundaries`);
  }
  ...
  const householdsByAdvisor = buildGroupIndex(dataset.households, "advisorId");
  ...
  const decisionsByHousehold = buildGroupIndex(optional.decisions, "householdId");
  const decisionsByAdvisor = buildGroupIndex(optional.decisions, "advisorId");
  const decisionsBySignal = new Map(optional.decisions.map((decision) => [decision.signalId, decision]));
  const plansByHousehold = buildGroupIndex(optional.actionPlans, "householdId");
  const plansByDecision = new Map(optional.actionPlans.map((plan) => [plan.decisionId, plan]));
  const actionsByPlan = buildGroupIndex(optional.actions, "planId");

  const stats = Object.freeze({
    ...Object.fromEntries(REQUIRED_COLLECTIONS.map((collection) => [collection, dataset[collection].length])),
    ...Object.fromEntries(OPTIONAL_COLLECTIONS.map((collection) => [collection, optional[collection].length])),
  });
  ...
  return Object.freeze({
    ...
    getInsight: (id) => insightsById.get(id) || null,
    getDecision: (id) => decisionsById.get(id) || null,
    getDecisionBySignal: (signalId) => decisionsBySignal.get(signalId) || null,
    getActionPlan: (id) => actionPlansById.get(id) || null,
    getActionPlanByDecision: (decisionId) => plansByDecision.get(decisionId) || null,
    listHouseholdIds: () => Object.freeze([...householdsById.keys()]),
    listAdvisorHouseholds: (advisorId) => householdsByAdvisor.get(advisorId) || EMPTY,
    ...
    listAdvisorDecisions: (advisorId) => decisionsByAdvisor.get(advisorId) || EMPTY,
    listHouseholdActionPlans: (householdId) => plansByHousehold.get(householdId) || EMPTY,
    listPlanActions: (planId) => actionsByPlan.get(planId) || EMPTY,
  });
}
