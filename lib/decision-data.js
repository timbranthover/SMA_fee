...
export async function loadHouseholdTimeline(householdId, { signal } = {}) {
  ...
  const local = listDecisionTransitions().filter((transition) => transition.householdId === householdId).map((transition, index) => ({
    id: `local-${transition.decisionId}-${transition.occurredAt}-${index}`,
