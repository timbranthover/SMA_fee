...
function priorityRank(insight, cashPct = 0) {
  if (!insight) return 0;
  if (insight.severity === "Opportunity" && insight.kind === "liquidity" && cashPct > 14) return 6;
  if (insight.tone === "red") return 5;
  if (insight.severity === "Upcoming" && /funding source not identified/i.test(insight.detail)) return 4.5;
...
function choosePriority(insights, cashPct = 0) {
  return [...insights].sort((left, right) => priorityRank(right, cashPct) - priorityRank(left, cashPct))[0] || null;
}
...
export function createWealthService(dataset, { cacheLimits = {}, projectionLimits = {}, repository: repositoryOverride = null } = {}) {
  ...
  function getHouseholdOverview(householdId) {
    ...
    return overviewCache.set(householdId, deepFreeze({
      ...
      insights: [...repository.listHouseholdInsights(householdId)].sort((left, right) => priorityRank(right, household.liquidityPct) - priorityRank(left, household.liquidityPct)).slice(0, projections.priorityInsights).map(projectInsight),
    }));
  }
  ...
  function buildBookHousehold(record) {
    ...
    const cashPct = financialAssets ? cash / financialAssets * 100 : 0;
    const priority = choosePriority(insights, cashPct);
    const priorityDecision = decisions.find((decision) => decision.sourceInsightId === priority?.id);
    const focus = [];
    ...
    return deepFreeze({
      ...
      decisionStatus: priorityDecision?.status || null,
      priority: priority ? { severity: priority.severity, tone: priority.tone, title: priority.title, detail: priority.detail, decisionId: priorityDecision?.id || null, decisionStatus: priorityDecision?.status || null } : null,
      priorityScore: priorityRank(priority, cashPct),
      focus,
      asOf: record.asOf,
