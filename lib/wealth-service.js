...
export function createWealthClientService(dataset) {
  ...
  function getHouseholdWorkspace(householdId) {
    ...
    const policy = repository.listHouseholdConcentrationPolicies(householdId).find((item) => item.isPrimary) || repository.listHouseholdConcentrationPolicies(householdId)[0] || null;
